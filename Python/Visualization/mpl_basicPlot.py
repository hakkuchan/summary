...
line1 = ax.plot(X, Y1, color='blue', marker='o', ms=3, ls='--', linewidth=1, label='sin')
# color: 点颜色 / marker: 点样式 / ms: 点大小 / ls: 线样式 / linewidth: 线宽 / label: 图例
line2 = ax.scatter(X, Y2, color='red', label='cos', s=10) # s: 点大小
...
# (3) 背景网格、标注、图例
ax.grid(True, linestyle="--", color="gray", linewidth=0.5, axis='both')
ax.text(-3, 1.5, 'Trigonometrics', fontdict={'size':14, 'color':'red'})  
ax.legend(loc='best', fontsize=14)  # 设置图例
# 图例位置：best, upper right, lower left 等 (upper, lower + left, center, right 组合)
plt.show()