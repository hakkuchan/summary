...
for train,test in kfold.split(X):
    ...
    print(model.predict(X_test))   # 模型预测
    print(model.predict_proba(X_test)[0])      # 把 X 预测为各个类型的概率,
    print(model.predict_log_proba(X_test)[0])  # 把 X 预测为各个类型的概率的对数值
    print(f'准确率：{model.score(X_test, y_test)}', '\n') # 计算准确率

out = model_selection.cross_val_score(model,X,y,cv=kfold)
print(f'平均准确率：{out.mean():.4f} ± {out.std():.4f}')
